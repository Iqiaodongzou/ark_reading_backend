...
main(Req0) ->
    ...
    case cowboy_req:read_body(Req0) of
        {ok, Data, _} ->
            ...
            ReqParams = Context;
		Other ->
            io:format("other data ~s", [Other]),
            ReqParams = []
    end,
    inets:start(),
    Link = <<"https://translation.googleapis.com/language/translate/v2?key=AIzaSyCtg5rcGLTp21J1aEFst12f1BqY3_97a_E">>,
    ReqBody = ReqParams,
    METHOD = post,
    ...
    {ok, {{_, 200, _}, _, Body}} =
	httpc:request(METHOD,{URL, AuthHeader,"application/json",jsx:encode(ReqBody)}, [], []),
    RespBody = jsx:decode(list_to_binary(Body),[{return_maps, false}]),
	RespBody,
	Body.
